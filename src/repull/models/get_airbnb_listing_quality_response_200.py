...
T = TypeVar("T", bound="GetAirbnbListingQualityResponse200")



@_attrs_define
class GetAirbnbListingQualityResponse200:
    """ 
        Attributes:
            data (Any): Shape depends on `type`: an object `{ standards, issues }` for `all`/`standards`, or an array for
                `issues`/`stats`.
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        get_airbnb_listing_quality_response_200 = cls(
            data=data,
            data_freshness=data_freshness,
        )


        get_airbnb_listing_quality_response_200.additional_properties = d
        return get_airbnb_listing_quality_response_200
