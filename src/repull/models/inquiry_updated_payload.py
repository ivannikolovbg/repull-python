...
class InquiryUpdatedPayload:
    ...
    `previousAttributes` holds only what moved, with prior values. Fires whether the host acted through the API, the
    Vanio dashboard or the Airbnb app. An inquiry whose dates simply pass is `expired` in `GET /v1/inquiries` but fires
    no event unless the channel reports it.
